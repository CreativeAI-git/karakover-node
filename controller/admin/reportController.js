...
const { profileimages, delete_group_report, get_user_data_by_id, get_user_language } = require('../../models/users');
const { base_url } = require('../../config');
const { send_notification_dynamicly } = require('../api/verificationController');
const { verification_passed_templetes, send_warning_notification_templetes } = require('../../utils/email_template_manager');
...
exports.get_all_report_user = async (req, res) => {
    try {
        ...
        const updatedReports = await Promise.all(reports.map(async (report) => {
            ...
            const receiver_report_count = 'SELECT COUNT(*) AS report_count FROM reports WHERE reciver_id = ?';
            const [receiver_report_count_here] = await db.query(receiver_report_count, [report.reciver_id]);
            let report_count = receiver_report_count_here.report_count;
            ...
            const senderProfileImages = await profileimages(sender.id);
            ...
            sender.images = senderProfileImages?.length > 0
                ? senderProfileImages.map(imageObj => imageObj.image ? `${base_url}/profile/${imageObj.image}` : "")
                : [];
        ...
        const updatedReports = await Promise.all(reports.map(async (report) => {
            ...
            try {
                ...
                if (group_data) {
                    ...
                    const receiver_report_count = 'SELECT COUNT(*) AS report_count FROM reports WHERE group_id = ?';
                    console.log(group_data.id);
                    
                    const [receiver_report_count_here] = await db.query(receiver_report_count, [group_data.id]);
                    report_count = receiver_report_count_here.report_count;
                }
            } catch (firebaseError) {
...
exports.send_warning_to_reported_user = async (req, res) => {
    try {
        const sendWarningSchema = Joi.object({
            user_id: Joi.string().required()
        })
        const { error, value } = sendWarningSchema.validate(req.body);
        if (error) return joiErrorHandle(res, error);
        const { user_id } = value;
        let [user] = await get_user_data_by_id(user_id)
        let [user_language] = await get_user_language(user_id)

        let language = user_language.language

        const { subject, template } = send_warning_notification_templetes(language);
        const emailTemplatePath = path.resolve(__dirname, template);
        const emailHtml = await ejs.renderFile(emailTemplatePath, { image_logo, name: user.name });
        ...
        return handleSuccess(res, 200, "Warning Sent Successfully");
    } catch (error) {
